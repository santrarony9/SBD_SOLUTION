                    ...
                    <div className="flex-grow overflow-y-auto p-8 custom-scrollbar bg-gray-50/30">
                        <form id="productForm" onSubmit={handleSubmit} className="grid grid-cols-12 gap-8">
                            ...
                            <div className="col-span-12">
                                <h3 className="text-[10px] uppercase font-black text-brand-navy tracking-[0.2em] mb-4 border-b pb-2 flex justify-between items-center">
                                    Product Assets
                                    <span className="text-gray-400 font-bold uppercase tracking-widest text-[9px]">Main (1) & Secondary Hover (2)</span>
                                </h3>
                                            ...
                                            {tempImages[index] ? (
                                                <div className="relative w-full h-full overflow-hidden rounded">
                                                    ...
                                                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                                        <button 
                                                            type="button" 
                                                            onClick={() => handleDownload(tempImages[index], index + 1)}
                                                            className="bg-white/90 text-brand-navy p-1.5 rounded-full hover:bg-brand-gold transition-colors shadow-sm"
                                                        ...
                                                        <button 
                                                            type="button" 
                                                            onClick={() => { const n = [...tempImages]; n[index] = ''; setTempImages(n); }} 
                                                ...
                                                <div className="w-full h-full flex items-center justify-center border border-dashed border-gray-200 rounded text-gray-300 relative hover:border-brand-gold hover:bg-brand-gold/5 transition-all">
                                                    <span className="text-[10px] font-black">{isUploading ? '...' : (index === 0 ? 'MAIN' : index === 1 ? 'HOVER' : index + 1)}</span>
                                                    <input 
                                                        type="file" 
                                                        accept="image/*" 
                                        ...
                                        {formData.coverImage ? (
                                            <div className="relative w-full h-full overflow-hidden rounded">
                                                ...
                                                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                                    <button 
                                                        type="button" 
                                                        onClick={() => handleDownload(formData.coverImage, 'hover')}
                                                        className="bg-white/90 text-brand-navy p-1.5 rounded-full hover:bg-brand-gold transition-colors shadow-sm"
                                                        title="Download Original"
                                                    >
                                                        <PiDownloadSimple size={14} />
