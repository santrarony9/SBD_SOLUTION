...
export default function SimilarPriceRange({ currentPrice, currentProductId }: { currentPrice: number, currentProductId: string }) {
    ...
    useEffect(() => {
        async function loadSimilarProducts() {
            ...
            try {
                // Calculate +/- 25% price range
                const minPrice = Math.floor(currentPrice * 0.75);
                const maxPrice = Math.ceil(currentPrice * 1.25);

                const data = await fetchAPI(`/products?minPrice=${minPrice}&maxPrice=${maxPrice}`);
                if (Array.isArray(data)) {
                    // Filter out the current product and take only up to 10 to keep it concise
    ...
    return (
        <section className="py-16 bg-brand-cream/30 border-t border-brand-gold/10">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex justify-between items-end mb-10">
                    <div>
                        <span className="text-brand-gold text-[10px] font-bold uppercase tracking-[0.3em] inline-block mb-2">Explore alternative options</span>
                        <h2 className="text-2xl md:text-3xl font-serif text-brand-navy">Similar Price Range</h2>
                    </div>
                    ...
                    {products.map((product) => (
                        ...
                        >
                            <Link href={`/product/${product.slug}`}>
                                <div className="relative aspect-square mb-4 bg-white overflow-hidden rounded-sm border border-brand-charcoal/5">
                                    {product.images?.[0] ? (
                                        <Image
                                ...
                                <div className="flex flex-col gap-0.5">
                                    ...
                                    <p className="text-xs font-bold text-brand-navy mt-1">
                                        {globalFormatPrice(product.price || product.pricing?.finalPrice || 0)}
                                    </p>
                                </div>
                            </Link>
                        </motion.div>
