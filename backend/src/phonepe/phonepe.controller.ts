...
export class PhonePeController {
  ...
  async initiatePayment(
    ...
    return await this.phonepeService.initiatePayment(
      body.orderId,
      body.amount,
      body.mobile,
      body.userId,
    ...
    try {
      if (body.response) {
        ...
        if (data.code === 'PAYMENT_SUCCESS') {
          ...
          const order = await this.ordersService.findByPhonePeTransactionId(
            merchantTransactionId,
          );

          if (order) {
            await this.ordersService.confirmPhonePePayment(
              order.id,
              merchantTransactionId,
  ...
  async handleRedirect(
    ...
    const frontendUrl =
      process.env.FRONTEND_URL || 'https://www.sparkbluediamond.com';
