...
export class MarketingService {
  ...
  async getGiftRecommendations(answers: {
    ...
    try {
      ...
      if (answers.budget === '0-25000') {
        maxPrice = 25000;
      } else if (answers.budget === '25000-50000') {
        minPrice = 25000;
        maxPrice = 50000;
      ...
      const candidates = products
        .filter((product) => {
          // Calculate Price
          const goldRate =
            goldRates.find((r) => r.purity === product.goldPurity)
              ?.pricePer10g || 0;
          const diamondRate =
            diamondRates.find((r) => r.clarity === product.diamondClarity)
              ?.pricePerCarat || 0;
          ...
          charges.forEach((charge) => {
            if (charge.type === 'FLAT') overhead += charge.amount;
              ...
              charge.applyOn === ApplyOn.SUBTOTAL
            )
              overhead += (subTotal * charge.amount) / 100;
          });
      ...
      if (candidates.length === 0) {
        ...
        return products.slice(0, 3).map((c) => ({
          ...c,
          imageUrl: c.images[0],
          matchReason: 'A beautiful choice from our collection.',
        }));
      }
          ...
          .map((rec: any) => {
            ...
            const diamondRate =
              diamondRates.find((r) => r.clarity === product.diamondClarity)
                ?.pricePerCarat || 0;
            const subTotal =
              (goldRate / 10) * product.goldWeight +
              diamondRate * product.diamondCarat;
            let overhead = 0;
            charges.forEach((charge) => {
              if (charge.type === 'FLAT') overhead += charge.amount;
              else if (
      ...
      return candidates.slice(0, 3).map((c) => {
        ...
        const goldRate =
          goldRates.find((r) => r.purity === c.goldPurity)?.pricePer10g || 0;
        const diamondRate =
          diamondRates.find((r) => r.clarity === c.diamondClarity)
            ?.pricePerCarat || 0;
        const subTotal =
          (goldRate / 10) * c.goldWeight + diamondRate * c.diamondCarat;
        return {
          ...c,
          price: Math.round(subTotal * 1.2),
          imageUrl: c.images[0],
          matchReason: 'Selected based on your preferences.',
        };
      });
    } catch (error) {
  ...
  async getRecentlyViewed(ipAddress: string) {
    try {
      ...
      const productIds = Array.from(
        new Set(
          activities.map((a: any) => a.metadata?.productId).filter(Boolean),
        ),
      ...
      const enrichedProducts = await Promise.all(
        productIds.map((id) =>
          this.productsService.findOne(id as string).catch(() => null),
        ),
      );
