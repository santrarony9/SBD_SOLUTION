...
export class InvoiceService {
  ...
  async generateInvoice(orderId: string): Promise<Buffer> {
    const order = await this.fetchOrder(orderId);
    return this.generatePDF((doc) => this.buildInvoice(doc, order), {
      size: 'A4',
    });
  }
  ...
  async generateCreditNote(orderId: string): Promise<Buffer> {
    const order = await this.fetchOrder(orderId);
    return this.generatePDF((doc) => this.buildCreditNote(doc, order), {
      size: 'A4',
    });
  }
  ...
  async generateLabel(orderId: string): Promise<Buffer> {
    const order = await this.fetchOrder(orderId);
    return this.generatePDF((doc) => this.buildLabel(doc, order), {
      size: [288, 432],
      margin: 20,
    });
  }
  ...
  async generateBulkPDF(
    orderIds: string[],
    type: 'invoice' | 'label' | 'credit-note',
  ): Promise<Buffer> {
  ...
  private generatePDF(
    builder: (doc: PDFKit.PDFDocument) => Promise<void> | void,
    options: any,
  ): Promise<Buffer> {
  ...
  private async buildLabel(doc: PDFKit.PDFDocument, order: any) {
    ...
    doc
      ...
      .fontSize(14)
      .font('Helvetica-Bold')
      .text('SPARK BLUE', 20, 20)
      .fontSize(8)
      .font('Helvetica')
      .text('LUXURY LOGISTICS', 20, 35);
    ...
    doc
      ...
      .text(
        `Weight: ${order.items.reduce((acc, current) => acc + (current.product.realWeight || 0), 0)}g`,
        20,
        285,
      );
    ...
    doc
      .fontSize(14)
      .font('Helvetica-Bold')
      .text('E-WAY BILL NOT REQ.', 20, 320, { align: 'center', width: 248 });

    doc
      .fontSize(8)
      .font('Helvetica')
      .text('Handle with care. High value contents.', 20, 380, {
        align: 'center',
        width: 248,
      });
  }
  ...
  private generateCustomerInfo(
    ...
    if (sectionTitle === 'Credit Note Details') {
      doc
        .text(`CN No: ${order.creditNoteNumber || 'PENDING'}`, 50, 160)
        .text(
          `Date: ${new Date(order.cancelledAt || Date.now()).toLocaleDateString()}`,
          50,
          175,
        )
        .text(
          `Ref Invoice: ${order.invoiceNumber || order.id.slice(-8).toUpperCase()}`,
          50,
          190,
        );
    } else {
      doc
        .text(
          `Invoice No: ${order.invoiceNumber || order.id.slice(-8).toUpperCase()}`,
          50,
          160,
        )
        .text(
        ...
        .text(
          `Place of Supply: ${order.placeOfSupply || shipping.state}`,
          50,
          190,
        );
    }
    ...
    if (order.customerGSTIN) {
      doc
        .fillColor('#D4AF37')
        .font('Helvetica-Bold')
        .text(`GSTIN (B2B): ${order.customerGSTIN}`, 50, 205)
        .fillColor('#000');
    }
  ...
  private async generateInvoiceTable(
    ...
    this.generateTableRow(
      ...
      '',
      'Subtotal',
      `${prefix}₹${order.totalAmount.toLocaleString()}`,
    );
    ...
    if (igst > 0) {
      ...
      this.generateTableRow(
        ...
        '',
        'SGST',
        `${prefix}₹${sgst.toFixed(2)}`,
      );
    }

    doc.font('Helvetica-Bold');
    const finalRowPos =
      igst > 0 ? subtotalPosition + 40 : subtotalPosition + 60;
    this.generateTableRow(
      doc,
      finalRowPos,
      '',
      ...
      '',
      '',
      isReversal ? 'Total Refund' : 'Grand Total',
      `${prefix}₹${totalWithTax.toLocaleString()}`,
    );
  }
  ...
  private generateFooter(doc: PDFKit.PDFDocument, order: any) {
    ...
    try {
      amountWords =
        'RUPEES ' +
        this.numberToWords(Math.round((order.totalAmount || 0) * 1.03)) +
        ' ONLY';
    } catch (e) {
    ...
    doc
      .fontSize(10)
      .font('Helvetica-Bold')
      .text('Amount in Words:', 50, doc.y + 40);
    doc
      .fontSize(10)
      .font('Helvetica')
      .text(amountWords, 50, doc.y + 10);

    doc
      .fontSize(10)
      .font('Helvetica-Bold')
      .text('Declaration:', 50, doc.y + 30);
    doc
      .fontSize(8)
      .font('Helvetica')
      .text(
