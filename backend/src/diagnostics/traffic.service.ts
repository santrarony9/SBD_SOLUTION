...
export class TrafficService {
  ...
  private cleanOldRequests() {
    ...
    if (this.requests.length > 1000) {
      ...
      while (
        ...
        this.requests[firstValid] < cutoff
      ) {
        firstValid++;
      }
      if (firstValid > 0) {
  ...
  getMetrics() {
    ...
    const totalInWindow = this.requests.filter(
      (t) => now - t < this.ADVISORY_WINDOW,
    ).length;
    ...
    return {
      ...
      status:
        rpm > this.CAPACITY_THRESHOLD
          ? 'Critical'
          : rpm > this.CAPACITY_THRESHOLD * 0.7
            ? 'Warning'
            : 'Healthy',
      isUpgradeRecommended,
      advice: isUpgradeRecommended
        ? 'High traffic sustained. Migrate to a VPS for 10,000 traffic capability.'
        : rpm > this.CAPACITY_THRESHOLD * 0.8
          ? 'Approaching capacity limits. Monitor closely.'
