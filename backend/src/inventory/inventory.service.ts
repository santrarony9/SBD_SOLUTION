...
export class InventoryService {
  ...
  async adjustStock(
    ...
    if (updated.stockCount <= 3) {
      this.whatsappService
        .sendAdminStockAlert(product.name, updated.stockCount)
  ...
  async updateMaterialStock(type: string, quantity: number, unit: string) {
    ...
    const existing = await this.prisma.materialStock.findFirst({
      where: { type },
    });
    if (existing) {
  ...
  async getInventoryValuation() {
    ...
    for (const product of products) {
      ...
      if (goldPrice && diamondPrice) {
        const productValue =
          product.goldWeight * (goldPrice.pricePer10g / 10) +
