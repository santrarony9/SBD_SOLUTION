...
export class ShiprocketService {
  ...
  async createOrder(orderData: any) {
    ...
    const payload = {
      ...
      order_date:
        ...
        ' ' +
        new Date(orderData.createdAt).toTimeString().split(' ')[0],
      pickup_location: 'Primary', // Default, user can change if needed
      billing_customer_name:
        orderData.user?.name || orderData.shippingAddress.fullName,
      billing_last_name: '',
  ...
  async generateAWB(shipmentId: string) {
    ...
    try {
      ...
      this.logger.log(
        `Shiprocket AWB Generated: ${response.data.response.data.awb_code}`,
      );
